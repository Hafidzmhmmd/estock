    ...
    <div class="row">
        <div class="col-3">
            <div class="card">
                <div class="body">
                    <div class="list-group list-widget">
                        @foreach ($gudang as $g )
                            @if($loop->first)
                                <a href="javascript:void(0);" class="list-group-item text-muted thisgudang" data-gudangid="{{$g->id}}">
                                    {{$g->nama_gudang}}
                                </a>
                            @else
                                <a href="javascript:void(0);" class="list-group-item text-muted" data-gudangid="{{$g->id}}">
                                    {{$g->nama_gudang}}
                                </a>
                            @endif
                        @endforeach
            ...
            <div class="card" id="takeoutHolder">
                ...
                <div class="card-body">
                    <div class="list-group list-widget" style="min-height: 200px">

                    </div>
                    <hr>
                    <button type="button" class="btn btn-block btn-outline-primary" data-toggle="modal" data-target=".form-pengambilan">Proses</button>
                </div>
            </div>
        </div>
        <div class="col-9">
            <div class="card">
                <div class="card-body">
                    <ul class="nav nav-tabs">
                        <li class="nav-item"><a class="nav-link active show" data-toggle="tab" href="#list-barang">Stock Gundang</a></li>
                        <li class="nav-item"><a class="nav-link" data-toggle="tab" href="#riwayat">Riwayat</a></li>
                        @if ($pengelolaGudang)
                            <li class="nav-item"><a class="nav-link" data-toggle="tab" href="#transfer">Transfer Stock</a></li>
                        @endif
                    </ul>
...
<script>
    var dtBarang = $('#tblstock').DataTable({
        ...
        columnDefs: [
            ...
            },
            {
                ...
                render: function(data, type, full, meta) {
                    if(full.stock > 0){
                        return (`<button type="button" class="btn btn-outline-primary" onclick="takeout(this)"><i class="icon-plus"></i></button>`);
                    } else {
    ...
    function takeout(elm){
        ...
        if(exist.length){
            ...
            let el = `<div class="takeout-items mb-2 border-bottom py-2" data-itemid ='${data.barang_id}' data-gudangid='${data.gudang_id}'>
                            ...
                            <div class="btn-group w-100">
                                ...
                                <span class="m-0  w-75 text-center p-2 ct-data" data-max='${data.stock}'><span class='item-count'>1</span> ${data.satuan}</span>
        ...
        if(ct >= data.stock){
            $(elm).parent().html('<button type="button" class="btn btn-outline-secondary" disabled><i class="icon-plus"></i></button>')
        }
    }
